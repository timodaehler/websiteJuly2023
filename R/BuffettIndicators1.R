...
p <- 
  ...
  add_trace(data = joined_data_swiss, x = ~Date, y = ~predicted, type = "scatter", mode = "lines", line = list(color = 'red'), name = "Exponential Trend Line") %>%
  add_trace(x = c(min(joined_data_swiss$Date), max(joined_data_swiss$Date)),
            y = c(median_value_swiss, median_value_swiss),
            type = "scatter", mode = "lines", line = list(color = 'black', dash = "dash"),
            name = "Median Value") %>%
  layout(
    xaxis = list(title = ""),
    yaxis = list(title = "Total SPI Market Cap to GDP Ratio"),
...
# Update date
saveRDS(Sys.time(), "content/project/BuffettIndicators/BuffettIndicators1_update_date.rds")
